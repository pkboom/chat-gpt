...
def get_recommendations(
    ...
    res = ""
    # chatgpt.generate_recommendations(
    #     scenario_name, mission_name, previous_conversation
    # )
...
def play_mission_one_full_branch(mission_id: str):
    ...
    for turn in this_mission_turns:
        # extract key phrase
        turn.key_phrase_en = ""  # chatgpt.extract_keyphrase(user_content)
...
if __name__ == "__main__":
    ...
    child_scenarios = airport_content[scene_num - 1]["child_scenarios"]
    child_scenarios_len = len(child_scenarios)

    for mission_num in range(len(child_scenarios)):
        print(child_scenarios[mission_num])

        child_scenario = child_scenarios[mission_num]
        # child_scenario = airport_content[scene_num - 1]["child_scenarios"][
        #     mission_num - 1
        # ]
        ...
        mission_obj["order"] = mission_num + 1
        mission_obj["turns"] = this_mission_turns_dict

        with open(
            f"airportTopics/scene{scene_num}/mission{mission_num + 1}e.json",
            "w",
            encoding="utf-8",
        ) as file:
        ...
        print(
            f"{mission_num + 1}/{child_scenarios_len} Completion time: {datetime.now().time()}"
        )
